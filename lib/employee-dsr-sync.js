...
const buildEmployeeRelevantProductNames = ({
  ...
  for (const report of existingReports) {
    if (report?.itemName) names.add(report.itemName)
  }
...
const buildEmployeeDailyAggregation = (date, groupedData, candidateNames = []) => {
  ...
  for (const assignment of assignments) {
    if (assignment.status !== "received") continue
  ...
  const acceptedReturns = groupedData.acceptedReturnsByDate.get(date) || []
  for (const transaction of acceptedReturns) {
  ...
  const [oldestSale, oldestCylinder, oldestRefill, oldestPurchase, oldestAssignment, oldestReturn, oldestSnapshot, oldestEmployeePurchase] =
    await Promise.all([
      ...
      StockAssignment.findOne(employeeId ? { employee: employeeId } : {})
        .sort({ receivedDate: 1, assignedDate: 1, createdAt: 1 })
        .select("receivedDate assignedDate createdAt")
        .lean(),
      ReturnTransaction.findOne(employeeId ? { employee: employeeId, status: "received" } : { status: "received" })
...
export async function recalculateEmployeeDailyStockReportsFrom(employeeId, startDateInput) {
  ...
  const [
    ...
    Product.find({ category: "cylinder" }).select("name").lean(),
  ...
  const groupedData = {
    ...
    acceptedReturnsByDate: groupByDate(returnTransactions, getEmployeeReturnDateValue),
  }
  ...
  return {
    ...
    sources: {
      ...
      acceptedReturns: returnTransactions.length,
    },
  }
}
