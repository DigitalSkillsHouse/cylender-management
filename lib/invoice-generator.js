...
// ReturnTransaction removed - return transactions don't use invoice numbers
 ...
 * Note: Return transactions don't use invoice numbers
 */
...
export async function verifyInvoiceUniqueness(invoiceNumber) {
  try {
    // Return transactions don't use invoice numbers, so we don't check them
...
export async function initializeInvoiceCounter() {
  try {
    ...
    if (configuredStartingNumber) {
      ...
      // Note: Return transactions don't use invoice numbers, so we don't check them
