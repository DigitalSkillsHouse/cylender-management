 ...
 * across all sales and transaction types (admin/employee gas sales, cylinder transactions, return transactions)
 */
...
export async function verifyInvoiceUniqueness(invoiceNumber) {
  try {
    const [saleExists, empSaleExists, cylinderExists, returnTransactionExists] = await Promise.all([
      Sale.findOne({ invoiceNumber }),
      EmployeeSale.findOne({ invoiceNumber }),
      CylinderTransaction.findOne({ invoiceNumber }),
      ReturnTransaction.findOne({ invoiceNumber })
    ])
    
    const isDuplicate = !!(saleExists || empSaleExists || cylinderExists || returnTransactionExists)
    ...
    if (isDuplicate) {
      ...
      console.error('Found in:', {
        ...
        returnTransaction: !!returnTransactionExists
      })
    }
...
export async function initializeInvoiceCounter() {
  try {
    ...
    if (configuredStartingNumber) {
      ...
      const [latestSale, latestEmpSale, latestCylinder, latestReturnTransaction] = await Promise.all([
        ...
        ReturnTransaction.findOne({ invoiceNumber: { $regex: /^\d{4}$/ } }).sort({ invoiceNumber: -1 })
      ])
      ...
      if (latestReturnTransaction?.invoiceNumber) {
        highestNumber = Math.max(highestNumber, parseInt(latestReturnTransaction.invoiceNumber) || 0)
      }
