...
export const GasSales = () => {
        ...
        const itemsNormalized: NormalizedItem[] = (saved?.items && Array.isArray(saved.items) && saved.items.length > 0)
          ...
          : saleItems.map((it) => {
              ...
              return { 
                ...
                price, 
                total, 
                category: category as "gas" | "cylinder",
                cylinderStatus: cylinderStatus as "empty" | "full" | undefined
              }
            })
  ...
  const updateItem = (index: number, field: string, value: any) => {
    ...
    // If productId is changed, handle the update atomically
      ...
      // Allow typing freely - validate on blur instead
      newItems[index] = {
        ...newItems[index],
        [field]: value,
      }
    ...
    if (enrichedSale.items && Array.isArray(enrichedSale.items)) {
      enrichedSale.items = enrichedSale.items.map((item: any) => {
	    ...
	    if (!sale.customerSignature && cached) {
	      ...
	      void persistSaleCustomerSignature(sale._id, cached)
	      return
	    }
  ...
  const handleSignatureComplete = async (signature: string) => {
    ...
    console.log('GasSales - Pending dialog type:', pendingDialogType)
    ...
    if (pendingSale) {
      const saleWithSignature = { ...pendingSale, customerSignature: signature }
	      ...
	      if (pendingSale?._id) {
	        ...
	        await persistSaleCustomerSignature(pendingSale._id, signature)
	      }
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-gray-500">Min AED {getSelectedRate(product._id, product.leastPrice).toFixed(2)}</span>
                                  </div>
                                </div>
                              </div>
                            ))
