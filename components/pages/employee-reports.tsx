...
export default function EmployeeReports({ user }: { user: { id: string; name: string; email: string; role: "admin" | "employee" } }) {
  ...
  // Download pending transactions PDF with proper header and footer (matching admin format)
  const downloadPendingTransactionsPdf = async () => {
    ...
    // Load employee signature from localStorage
    const employeeSignature = typeof window !== 'undefined' ? localStorage.getItem("employeeSignature") : null;
    ...
    const jsPDFModule = await import("jspdf");
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        headerImg.src = "/images/Customer-Ledger-header-pending.jpg";
	      ...
	      const filterText = filters.customerName.trim() !== '' 
	        ...
	        : `Employee: ${user?.name || 'N/A'} - All Customers with Pending Transactions`;
      ...
      // Process each customer (same structure as admin)
      for (const customer of pendingCustomers) {
        ...
        grandTotal += customerTotal;
	        ...
	        const minBlockHeight = hasAnyRows ? 24 : 14;
	        if (currentY + minBlockHeight > pageHeight - footerSpace) {
	          pdf.addPage();
	          currentY = margin + 2;
	        }

	        // Customer header (same styling as admin)
	        ...
	        pdf.text(customer.name, margin + 4, currentY + 5);
	        
	        pdf.setFontSize(7);
	        pdf.setFont(undefined, 'normal');
	        pdf.setTextColor(102, 102, 102);
	        pdf.text(`TR Number: ${customer.trNumber}`, margin + 4, currentY + 7.2);
          ...
          // Add cylinder transactions
	        ...
	        currentY += 2; // Space between customers
	      }
      ...
      pdf.setFont(undefined, 'bold');
		      ...
		      // Add footer image and employee signature (same structure as admin)
        ...
        footerImg.onload = async () => {
          try {
            ...
            canvas.height = footerImg.height;
              ...
              // Add employee signature on bottom right
              if (employeeSignature) {
                try {
                  await new Promise<void>((sigResolve, sigReject) => {
                    ...
                    signatureImg.onerror = () => sigReject(new Error("Failed to load signature"));
                    signatureImg.src = employeeSignature;
                  });
                } catch (sigError) {
                  pdf.setFontSize(8);
                  pdf.setTextColor(43, 48, 104);
                  pdf.setFont(undefined, 'bold');
		                  pdf.text("Employee Signature", pageWidth - margin - 30, footerY + actualFooterHeight - 8, { align: "center" });
                }
              } else {
                pdf.setFontSize(8);
                pdf.setTextColor(43, 48, 104);
      ...
      const fileName = buildPdfFileName({
        ...
        label: `Employee Pending Transactions ${stamp}`,
        fallbackName: `Employee Pending Transactions ${stamp}`,
      })
      pdf.save(fileName);
    ...
    // then fetch updated record and open receipt.
	    if (pendingReceiptData) {
	      ...
	      if (kind === "sale") {
	        ...
	        if (!ok) console.warn("Failed to persist customer signature for employee sale from employee reports")
	      }
  ...
  const fetchReportsData = async (loadCustomers = false) => {
    try {
      ...
      sales.forEach(s => {
        ...
        // Update customer data if missing (prefer non-empty values)
        if (s.customer) {
          if (!item.trNumber && s.customer.trNumber) item.trNumber = s.customer.trNumber
          if (!item.address && s.customer.address) item.address = s.customer.address
          if (!item.phone && s.customer.phone) item.phone = s.customer.phone
          if (!item.email && s.customer.email) item.email = s.customer.email
        }
      ...
      cylinders.forEach(c => {
        ...
        // Update customer data if missing (prefer non-empty values)
        if (c.customer) {
          if (!item.trNumber && c.customer.trNumber) item.trNumber = c.customer.trNumber
          if (!item.address && c.customer.address) item.address = c.customer.address
          if (!item.phone && c.customer.phone) item.phone = c.customer.phone
          if (!item.email && c.customer.email) item.email = c.customer.email
        }
